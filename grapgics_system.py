...
class GraphicsSystem:
    ...
    def _create_controls(self):
        ...
        ttk.Button(view_frame, text="+ Zoom", 
                 command=lambda: self.zoom_manual(0.9)).pack(side=tk.LEFT, padx=2)
        ttk.Button(view_frame, text="- Zoom", 
                 command=lambda: self.zoom_manual(1.1)).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(move_frame, text="↑", 
                 command=lambda: self.move_window('up')).grid(row=0, column=1, padx=2)
        ttk.Button(move_frame, text="↓", 
                 command=lambda: self.move_window('down')).grid(row=2, column=1, padx=2)
        ttk.Button(move_frame, text="←", 
                 command=lambda: self.move_window('left')).grid(row=1, column=0, padx=2)
        ttk.Button(move_frame, text="→", 
                 command=lambda: self.move_window('right')).grid(row=1, column=2, padx=2)

        # Controles de Objetos e Entrada
        obj_frame = ttk.Frame(control_frame)
        obj_frame.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        ttk.Button(obj_frame, text="Ponto", 
                 command=self.add_point).pack(side=tk.LEFT, padx=2)
        ttk.Button(obj_frame, text="Linha", 
                 command=self.add_line).pack(side=tk.LEFT, padx=2)
        ttk.Button(obj_frame, text="Polígono", 
        ...
        clear_frame.pack(side=tk.RIGHT, padx=5)
        
        ttk.Button(clear_frame, text="Apagar Selecionado", 
                command=self.delete_selected).pack(side=tk.RIGHT, padx=2)
        ttk.Button(clear_frame, text="Limpar Tudo", 
                command=self.clear_canvas).pack(side=tk.RIGHT, padx=2)
    ...
    def _bind_events(self):
        ...
        self.root.bind("<Up>", lambda e: self.move_window('up'))
        self.root.bind("<Down>", lambda e: self.move_window('down'))
        self.root.bind("<Left>", lambda e: self.move_window('left'))
        self.root.bind("<Right>", lambda e: self.move_window('right'))
